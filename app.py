...
CYCLE_STATE = {"running": False, "started": None}
...
.chain{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin:12px 0;
 padding:9px 11px;background:#f4f7ff;border:1px solid #cfdcfa;border-radius:8px;font-size:12.5px}
.chain-k{font-size:10.5px;font-weight:700;color:var(--accent);text-transform:uppercase;
 letter-spacing:.06em;margin-right:2px}
.chain .step{font-weight:560;color:#2a3342}
.chain .arrow{color:#8fa8dd}
...
def render_chain(trace):
    """Always-visible one-line summary of the agent chain.

    The full trace stays collapsible so it does not bury the clause citation,
    but a judge should be able to see that three agents and a deterministic
    pricing step ran without having to click anything.
    """
    if not trace:
        return ""
    steps, seen = [], set()
    for t in trace:
        name = t["tool"]
        if name in ("get_scope_of_work", "get_client_context"):
            label, key = "ScopeAnalyst", "analyst"
        elif name == "Verifier":
            label, key = ("Verifier ✓" if "agrees" in t["result"] else "Verifier ✗"), "verifier"
        elif name == "calculate_change_order":
            label, key = "pricing (deterministic)", "price"
        elif name == "Drafter":
            label, key = "Drafter", "drafter"
        else:
            continue
        if key in seen:
            continue
    ...
    if not steps:
        return ""
    chain = ' <span class="arrow">→</span> '.join(f"<span class='step'>{e(s)}</span>" for s in steps)
    return f'<div class="chain"><span class="chain-k">Strands chain</span>{chain}</div>'
...
def dashboard():
    ...
    running_note = ("<b>Agent is working now</b> — refresh to see progress · "
                    if CYCLE_STATE["running"] else "")
...
<form method="post" action="/cycle" class="bar">
  ...
  <span class="small">{running_note}Runs automatically every 10 minutes · last run: {e(last)}</span>
</form>
...
def _run_cycle_background():
    """Run a cycle off the request thread.

    A cycle makes several model calls per message, which is far too slow to
    hold an HTTP request open. Running it in the background is also truer to
    the product: the agent works while nobody is watching, and the dashboard
    simply reflects whatever it has finished so far.
    """
    if CYCLE_STATE["running"]:
        return
    CYCLE_STATE["running"] = True
    CYCLE_STATE["started"] = datetime.now(timezone.utc).strftime("%H:%M:%S UTC")
    try:
        res = agent.run_cycle()
        LAST_CYCLE["result"] = res
    except Exception as exc:  # noqa: BLE001
        LAST_CYCLE["result"] = {"error": f"{type(exc).__name__}: {exc}"}
        store.log_audit("error", f"cycle crashed: {type(exc).__name__}: {exc}")
    finally:
...
def _start_cycle():
    threading.Thread(target=_run_cycle_background, daemon=True).start()
...
@app.get("/run-cycle")
def run_cycle_get():
    """GET trigger so the cycle can be started by a cron ping or a plain link."""
    _start_cycle()
    return JSONResponse({"started": True, "note": "cycle running in background; poll /healthz"})
...
@app.get("/selftest")
def selftest():
    """Try each candidate Gemini model with one trivial call and report the
    result. Diagnostic only - it never returns the API key."""
    if not agent.provider_ready():
        return JSONResponse({"error": "no provider credentials configured"})
    results = [agent.try_model(m) for m in agent.CANDIDATE_MODELS]
    working = [r["model"] for r in results if r.get("ok")]
    return JSONResponse({
        "current_default": agent.DEFAULT_GEMINI_MODEL,
...
def healthz():
    ...
    # Diagnostic: report the NAMES of any configured keys (never the values),
    # so a misnamed environment variable can be identified without exposing secrets.
    key_names = sorted(
        k for k in os.environ
        if "GEMINI" in k.upper() or "API_KEY" in k.upper() or "SCOPEGUARD" in k.upper()
    )
    gk = os.environ.get("GEMINI_API_KEY", "")
    return JSONResponse({
        "status": "ok" if db_ok and agent.provider_ready() else "degraded",
        "database": "ok" if db_ok else "error",
        "env_keys_seen": key_names,
        "gemini_key_length": len(gk),
        "gemini_key_has_whitespace": gk != gk.strip(),
        ...
        "cycle_running": CYCLE_STATE["running"],
        "cycle_started": CYCLE_STATE["started"],
        "last_cycle": LAST_CYCLE["at"],
        "last_cycle_result": LAST_CYCLE["result"],
        "recent_errors": [a["detail"][:300] for a in store.audit_log(40)
                          if a["event"] == "error"][:5],
        "model_id": agent.DEFAULT_GEMINI_MODEL,
        "stats": s,
    })
