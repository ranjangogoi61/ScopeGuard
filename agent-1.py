...
def analyse_message(msg):
    ...
    analyst = _agent(SCOPE_ANALYST_PROMPT,
                     [get_scope_of_work, get_client_context], ScopeVerdict)
    verdict = analyst.structured_output(
        ScopeVerdict,
        f"Client ID: {client_id}\n\nIncoming message from {msg['sender']}:\n\"{msg['body']}\"\n\n"
        f"Read the scope of work for client_id '{client_id}' and decide."
    )
    ...
    verifier = _agent(VERIFIER_PROMPT, [get_scope_of_work], VerifierVerdict)
    check = verifier.structured_output(
        ...
        f"Read the scope of work for client_id '{client_id}' and argue against escalating."
    )
    ...
    drafter = _agent(DRAFTER_PROMPT, [get_client_context], DraftOutput)
    draft = drafter.structured_output(
        ...
        f"Look up the client context for client_id '{client_id}' and write the reply."
    )
